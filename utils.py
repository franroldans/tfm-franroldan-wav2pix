...
class Logger(object):
    ...
    def log_iteration_wgan(self, epoch, gen_iteration, d_loss, g_loss, real_loss, fake_loss):
        ...
        print("Epoch: %d, Gen_iteration: %d, d_loss= %f, g_loss= %f, real_loss= %f, fake_loss = %f" %
              (epoch, gen_iteration, d_loss.data.cpu().mean(), g_loss.data.cpu().mean(), real_loss.data.cpu().numpy(), fake_loss.data.cpu().numpy()))
        self.hist_D.append(d_loss.data.cpu().mean())
        self.hist_G.append(g_loss.data.cpu().mean())
        self.logger.info("Epoch: %d, Gen_iteration: %d, d_loss= %f, g_loss= %f, real_loss= %f, fake_loss = %f" %
                         (epoch, gen_iteration, d_loss.data.cpu().mean(), g_loss.data.cpu().mean(), real_loss.data.cpu().numpy(), fake_loss.data.cpu().numpy()))
    ...
    def log_iteration_gan(self, epoch, d_loss, g_loss, real_score, fake_score):
        ...
        self.logger.info("Epoch: %d, d_loss= %f, g_loss= %f, D(X)= %f, D(G(X))= %f" %
                         (epoch, d_loss.data.cpu().mean(), g_loss.data.cpu().mean(), real_score.data.cpu().mean(),
                          fake_score.data.cpu().mean()))

    def log_iteration_began(self, iteration, epoch, d_loss, g_loss, real_loss, fake_loss, lr):
        print("Step: %d, Epochs: %d, Loss D: %.9f, real_loss: %.9f, fake_loss: %.9f, Loss G: %.9f, lr:%.9f" % (iteration, epoch,
        d_loss.data[0], real_loss.data[0], fake_loss.data[0], g_loss.data[0],  lr))
        self.hist_D.append(d_loss.data.cpu().mean())
        self.hist_G.append(g_loss.data.cpu().mean())
        self.logger.info("Step: %d, Epochs: %d, Loss D: %.9f, real_loss: %.9f, fake_loss: %.9f, Loss G: %.9f, lr:%.9f" % (iteration, epoch,
        d_loss.data[0], real_loss.data[0], fake_loss.data[0], g_loss.data[0],  lr))
